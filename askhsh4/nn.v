...
module nn #(parameter SIZE = 32) (
    ...
    always @(*) begin
        ...
        case (state)
            ...
            PRE_PROC: begin
                if(total_ovf) next_state = IDLE;
                else next_state = INPUT_LAYER;
            end

            INPUT_LAYER: begin
                if(total_ovf) next_state = IDLE;
                else next_state = OUTPUT_LAYER;
            end

            OUTPUT_LAYER: begin
                if(total_ovf) next_state = IDLE;
                else next_state = POST_PROC;
            end
    ...
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            if(state == DEACTIVATED) begin
                ...
                end else if(alu_ovf1 || alu_ovf2) begin
                    total_ovf <= 1;
                    ovf_fsm_stage <= PRE_PROC;
                    final_output <= 32'hffffffff;
                end
                inter_1 <= alu_out1;
                inter_2 <= alu_out2;
                ...
                if (alu_ovf1 || m1_om || m1_oa || m2_om || m2_oa) begin
                    ...
                    final_output <= 32'hffffffff;//if ovf set result the max value
                end else begin
