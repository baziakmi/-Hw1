...
module tb_nn();
    ...
    initial begin
        ...
        for (i = 0; i < 100; i = i + 1) begin
            ...
            run_test($signed($urandom_range(8191)) - 4096, $signed($urandom_range(8191)) - 4096);

            // TEST 2: Πιθανή θετική υπερχείλιση
            run_test($signed($urandom_range(max_pos, max_pos/2)), $signed($urandom_range(max_pos, max_pos/2)));

            // TEST 3: Πιθανή αρνητική υπερχείλιση
            run_test(-$signed($urandom_range($abs(max_neg), $abs(max_neg/2))), -$signed($urandom_range($abs(max_neg), $abs(max_neg/2))));
        end
